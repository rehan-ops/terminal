...
bool OutputStateMachineEngine::ActionOscDispatch(const size_t parameter, const std::wstring_view string)
{
    ...
    case OscActionCodes::SetColor:
    {
        ...
        success = _GetOscSetColorTableStrided(string, tableIndexes, colors, 0, true);
    ...
    case OscActionCodes::SetHighlightColor:
    {
        ...
        success = _GetOscSetColorTableStrided(string, tableIndexes, colors, resource, false);
        if (success)
        {
            for (size_t i = 0; i < tableIndexes.size(); i++)
            {
                ...
                if (rgb == COLOR_INQUIRY_COLOR)
                {
                    success = success && _dispatch->RequestXtermColorResource(tableIndex);
                }
                else if (rgb != INVALID_COLOR)
                {
                    success = success && _dispatch->SetXtermColorResource(tableIndex, color);
                }
                resource++;
            }
        }
